...
if __name__ == '__main__':
    ...
    input = torch.randn((1, 16, 32, 32, 32)).cuda()
    input1 = torch.randn((1, 8, 32, 32, 32)).cuda()
    imput3 = torch.cat((input, input1), dim=1)
    x_ray = torch.randn((2, 1, 128, 128)).cuda()
    time = torch.randn((1,)).cuda()
    out_hor = model(x=input, time=time)
    print(out_hor.shape)

    from thop import profile
    from thop import clever_format

    flops, params = profile(model, inputs=(input, time))
