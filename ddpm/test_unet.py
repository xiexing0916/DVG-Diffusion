...
class Unet3D(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.concat.append(nn.ModuleList([
    ...
    def forward(
        ...
        for block11, block22, spatial_attnn, temporal_attnn, downsamplee in self.downs:
            x2 = block11(x2, t)
            x2 = block22(x2, t)
            # x = spatial_attn(x)
            # x = temporal_attn(x, pos_bias=time_rel_pos_bias,
        ...
        for block11, block22, spatial_attnn, temporal_attnn, upsamplee in self.ups:
            x2 = torch.cat((x2, h2.pop()), dim=1)
            x2 = block11(x2, t)
            x2 = block22(x2, t)
            # x = spatial_attn(x)
            # x = temporal_attn(x, pos_bias=time_rel_pos_bias,
        ...
        x2 = torch.cat((x2, r2), dim=1)
        x2 = self.final_conv(x2)
        n.append(x2)

        for block111, block222, spatial_attnnn, temporal_attnnn, upsampleee in self.concat:
            x3 = m.pop(0) + n.pop(0)
            x3 = block111(x3, t)
            x3 = block222(x3, t)
            # x = spatial_attn(x)
...
if __name__ == '__main__':
    ...
    input2 = torch.randn((1, 1, 128, 128, 128)).cuda()
    x_ray = torch.randn((2, 1, 128, 128))
    cond = torch.randn((1, 3, 128, 128))
    time = torch.randn((1,)).cuda()
    out_hor = model(x=input, x2=input2, time=time)
    print(out_hor.shape)
